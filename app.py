...
# --- 4. GENERACIÓN DINÁMICA DE CSS PARA FADERS ---
css_faders = ""
...
# --- 5. ESTILOS CSS GENERALES (MEJORA DE LEGIBILIDAD) ---
st.markdown(f"""
<style>
    .stApp {{
    ...
    /* Títulos de categoría en Blanco para máxima visibilidad */
    .category-label {{ 
        font-weight: bold; font-size: 18px; color: #FFFFFF !important; margin-top: 10px;
        text-shadow: 1px 1px 2px black;
    }}
    /* Subtítulos en celeste brillante */
    .stCaption p {{
...
with st.container():
    ...
    with st.expander("🎼 1. ALINEACIÓN (CONSOLA DE GÉNEROS)"):
        ...
        for g, color_hex in colores_generos.items():
            ...
            with cols[0]:
                ...
                vol = st.slider(g, 0, 4, 0, key=f"vol_{g_clean}", label_visibility="collapsed")
            for i in range(num_jugadores):
                with cols[i+1]:
    ...
    puntos_tracks = []
    with cols_t[0]: st.caption("PV de la ficha más baja (Máx 12)")
    for i in range(num_jugadores):
        with cols_t[i+1]:
            with st.popover("📍 Editar"):
                t1 = st.number_input("Amenidad", 0, 12, key=f"tr1_{i}")
                t2 = st.number_input("Asistencia", 0, 12, key=f"tr2_{i}")
                t3 = st.number_input("Precio", 0, 12, key=f"tr3_{i}")
    ...
    # --- SECCIÓN EXTRAS (COMBO EXPANSIBLE) ---
    with st.expander("➕ PUNTOS EXTRAS"):
...
# --- 8. PANEL DE RESULTADOS ALINEADO ---
st.markdown("---")
